...
//
// smpirun --cfg=smpi/bcast:mpich -np 50 -platform ring_50.xml -hostfile hostfile_50.txt ./bcast_skeleton naive_bcast
//
...
// Seed for the (bad) default random number generator
#define RAND_SEED 842270

// Number of bytes to broadcast (added ifdef for testing)
#ifndef NUM_BYTES
//#define NUM_BYTES 100000000
  #define NUM_BYTES 10000000  // smaller amount for testing on local machine
//#define NUM_BYTES 100  	  // even smaller amount for index testing
#endif
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if ((0 == rank) && (all_ok)) {
    fprintf(stdout,"\n\n*************************************************************************** implementation: %s | chunksize: %d |  time: %.3lf seconds\n\n\n\nDONE\n",
		    bcast_implementation_name, 
		    chunk_size,
